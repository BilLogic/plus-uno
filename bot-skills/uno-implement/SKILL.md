...
migration_notes: >
  model_default reverted to claude-sonnet-4-20250514 (current production ID)
  for zero-change migration. Bump to claude-sonnet-4-6 only after verifying
  that ID is a live Anthropic model — do not bump and test in the same step.
---
...
- A designer asks the bot to implement a change from a fresh PRD or change description in Slack (slack-direct, not gated on polling)
...
| Notion PRD (auto-looked-up by component name when present; see `findPRDByComponent()` in `scripts/create-notion-prd.js`) | Notion database | Looked up automatically |
