...
| **ratchet** | the harness's one reader and writer of a baseline record (`scripts/lib/ratchet.mjs`): the record's envelope (its `why`, the keyed set, the reason per entry), the shrink-only invariant, the stale entry a fix left behind, the entry nobody has written a reason for, and the `--update` write. A recorded count may FALL and only fall, and an absent record is one stated failure rather than one per check. What stays with a check is the wording of its verdict; one conformance suite holds every check's ratchet to the same invariant | "baseline", "threshold" (a baseline is the RECORD; a threshold stays put) |
