...
Load order is a bundle-level fact, declared once in the bundler's `SECTIONS` list. **167,271 chars from 16 files**, against an assembled budget of 170,000 (2,729 to spare), and a floor of 16,384 + 4,000 (implicit cache, GEMINI_REGION global), 146,887 above it. The floor is the minimum the cache in force will hold — Google's implicit cache on the `global` endpoint, the explicit `cachedContents` cache on a regional one — chosen by `GEMINI_REGION` in `agents/uno-bot/wrangler.toml`; a bundle cut under it ships uncached.

| # | Section | Doc | Chars | Running total | Budget |
|--:|---------|-----|------:|--------------:|--------|
| 1 | constitution | [`AGENTS.md`](../../AGENTS.md) | 9,603 (−7,277 ide-only) | 9,603 | 20,000 (constitution) |
| 2 | constitution | [`CONTEXT.md`](../../CONTEXT.md) | 14,843 | 24,474 | 20,000 (constitution) |
| 3 | persona | [`agents/uno-bot/AGENT.md`](../../agents/uno-bot/AGENT.md) | 27,187 | 51,702 | 28,000 (persona) |
| 4 | skills | [`skills/uno-maintain/bot.md`](../../skills/uno-maintain/bot.md) | 6,349 | 58,095 | 7,000 (Worker face) |
| 5 | skills | [`skills/uno-prototype/bot.md`](../../skills/uno-prototype/bot.md) | 6,999 | 65,139 | 7,000 (Worker face) |
| 6 | skills | [`skills/uno-publish/bot.md`](../../skills/uno-publish/bot.md) | 6,670 | 71,852 | 7,000 (Worker face) |
| 7 | skills | [`skills/uno-research/bot.md`](../../skills/uno-research/bot.md) | 5,243 | 77,139 | 7,000 (Worker face) |
| 8 | skills | [`skills/uno-review/bot.md`](../../skills/uno-review/bot.md) | 6,661 | 83,842 | 7,000 (Worker face) |
| 9 | skills | [`skills/uno-synthesize/bot.md`](../../skills/uno-synthesize/bot.md) | 6,356 | 90,244 | 7,000 (Worker face) |
| 10 | connectors | [`docs/connectors/figma.md`](../../docs/connectors/figma.md) | 1,929 (−4,709 ide-only) | 92,215 | — |
| 11 | connectors | [`docs/connectors/notion.md`](../../docs/connectors/notion.md) | 13,955 (−4,814 ide-only) | 106,213 | — |
| 12 | connectors | [`docs/connectors/slack.md`](../../docs/connectors/slack.md) | 14,280 | 120,535 | — |
| 13 | connectors | [`docs/connectors/supabase/blueprint-navigation.md`](../../docs/connectors/supabase/blueprint-navigation.md) | 3,182 | 123,783 | — |
| 14 | connectors | [`docs/connectors/supabase/blueprint.md`](../../docs/connectors/supabase/blueprint.md) | 30,532 | 154,370 | — |
| 15 | connectors | [`docs/connectors/supabase/overview.md`](../../docs/connectors/supabase/overview.md) | 4,535 (−1,219 ide-only) | 158,959 | — |
| 16 | engineering | [`docs/engineering/operations.md`](../../docs/engineering/operations.md) | 8,264 (−605 ide-only) | 167,271 | — |
...
| Harness gate | **every `pull_request`** + on demand | the deterministic guards, composed | — | `check-harness.yml` → `npm run check:harness` → `scripts/check-harness.mjs` (composition and the reason for each member live there; `--list` prints them). ~40s, of which the composite is ~14s; one `npm ci` scoped to `agents/uno-bot`, which the Worker's `typecheck` and `test` rows need | GHA · no model call — deterministic scripts only | Bill | ✅ live (#155; the repo's first `pull_request` trigger) |
| uno-bot Worker checks | **every `pull_request`** + on demand | the Worker's own typecheck and its 692 unit tests | — | `uno-bot-checks.yml` — two jobs, `typecheck` and `tests`, so a type failure and a test failure read apart in the PR's check list (#580). Both commands are also registry rows composed into `check:harness`, which is what keeps the pre-push command whole; the deploy chain runs them too, so a direct push to `main` meets them. No secret, so a fork PR runs it | GHA · no model call — `tsc` and `node --test` | Bill | ✅ live (#580) |
