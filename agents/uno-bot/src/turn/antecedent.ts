// The narrow conversation window: what came just BEFORE a root @-mention, so
// "this" has something to point at.
//
// THE HOLE IT FILLS
// -----------------
...
// thread reply, so the thread itself returns exactly one message: the
// @mention. The bot has never seen the thing being pointed at, and the
...
//
// LIVES HERE because the window is what the MODEL reads, not what a person
// sees. The Slack adapter fetches the page (`TurnDeps.readAntecedent`); Turn
// decides whether to ask and how to frame it. Slack's `antecedent.ts` re-exports
// this file so existing tests keep their import path.
//
 ...
 *
 * @param text the question as the person wrote it
 */
 ...
 *
 * @param messages neighbours, oldest first
 * @param maxChars newest-first budget for the kept lines
 */
