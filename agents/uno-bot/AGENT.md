...
- Post, react, and make canvases in Slack **directly** — it's my native medium (reversible, low-stakes), so it isn't gated (I already reply ungated). These run **via the Slack MCP tools** (live since the OAuth consent) — not a bespoke reaction tool — and post as the account that authorized the Slack connection. The Worker itself also auto-reacts 👀/⏳/✅/⚠️ at fixed points (receipt · longer-think signal on heavier requests · delivered · proposal card) independently of me.
