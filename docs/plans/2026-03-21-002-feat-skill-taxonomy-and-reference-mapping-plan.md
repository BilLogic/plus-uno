...
# Design /uno:xxx Skill Taxonomy with Reference Mapping

## Overview

This plan designs the 4 `/uno:xxx` skills for plus-uno: their purpose, phases, directory structure, references, and validation gates. Each skill is a user-invocable workflow triggered with `/uno:xxx`, orthogonal to the DS mode router (`docs/design-system/router.md`).

**Key architectural insight:**
- **Modes** (`docs/design-system/router.md`) = HOW to approach work (learning, maintaining, consulting, iteration, finalization)
- **Skills** (`/uno:xxx`) = WHAT lifecycle workflow to follow (prototype, compound, review, post)
...
|----------|--------|-----------|
| **Prototyping** | `/uno:prototype` | Not covered by any mode. Project-level workflow. Done frequently by Bill, Ashley, Victor. |
| **Compound learning** | `/uno:compound` | Not covered by any mode. The loop that makes everything else improve. |
| **Quality review** | `/uno:review` | Cross-cuts all modes. "Automatic validation is the most valuable pattern." |
| **Marketplace posting** | `/uno:post` | Not covered by any mode. Distinct lifecycle for the recently set up project marketplace. |
...
| 5. Report | Present pass/fail checklist. Auto-fix what's possible, escalate what isn't. Suggest `/uno:compound` if new gotcha found. | All passes or action items listed |
...
User invokes /uno:compound
  → Skill defines lifecycle (summarize → document → evaluate → propagate)
  → May trigger updates to AGENTS.md that affect future /uno:review runs

User invokes /uno:post
  → Skill defines lifecycle (identify → package → review → publish)
  → Natural follow-up after /uno:prototype or Finalization mode work
```
...
| Shared Reference | /uno:prototype | /uno:compound | /uno:review | /uno:post |
|-----------|:---:|:---:|:---:|:---:|
...
- **Risk**: 4 skills may be too many for early stage
  - **Mitigation**: Start with `/uno:prototype` and `/uno:compound`, add `/uno:review` and `/uno:post` after compound loop has surfaced patterns
- **Risk**: `/uno:post` marketplace infrastructure is new and may change
