...
- ~~`playground/` survivors invisible to the link guard~~ **resolved** (grep-verified clean 2026-08-01): `design-system/figma/component-alignment.md:72`, `.cursor/hooks/uno-writeback/storage.mjs:133` (live agent instruction!), usage examples in `scripts/audit-figma-writeback.mjs:4` + `validate-figma-writeback-manifest.mjs:4`, `skills/uno-review/SKILL.md:43` prose, `docs/evals/scenarios/uno-prototype.md:7`.
...
- ~~`scripts/figma-variables-snapshot.json:2` phantom generator/npm task~~ **resolved 2026-08-01** — note rewritten to the real consumer and manual refresh path.
- ~~559 lines of completed one-shot `migrate-*.mjs` scripts~~ **resolved 2026-08-01** — deleted (recoverable from git).
