...
5. **The validator existed but wasn't green.** `scripts/validate-doc-links.sh` would have caught most of #3 — it was failing on node_modules noise and two over-greedy patterns, so nobody ran it. Fixed (excludes `agents/*/node_modules`, allows `design-system/guidelines/figma/`); it now passes clean.
