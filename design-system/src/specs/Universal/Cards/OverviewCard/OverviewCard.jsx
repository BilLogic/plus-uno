...
const OverviewCard = ({
    ...
    useEffect(() => {
        if (!animateIntro) {
            setAnimatedChartValue(prev => prev === chartValue ? prev : chartValue);
            setAnimatedMetricLabel(prev => prev === targetMetricLabel ? prev : targetMetricLabel);
            setAnimatedSmartData(prev => {
                if (prev === smartData) return prev;
                // Simple comparison for SMART data object
                if (prev && smartData &&
                    prev.socio === smartData.socio &&
                    prev.mastering === smartData.mastering &&
                    prev.advocacy === smartData.advocacy &&
                    prev.relationships === smartData.relationships &&
                    prev.technology === smartData.technology) {
                    return prev;
                }
                return smartData;
            });
