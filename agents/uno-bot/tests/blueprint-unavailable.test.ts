...
// Supabase credentials at all, so there is nothing to reach. Every OTHER
// failure of the tool reports `reason: "unreachable"`, which tells the model
// the source of truth exists and could not be read this turn. Collapsing the
// two loses the only actionable one: "this deployment was never wired up" is a
// fact about the Worker's config, not about the blueprint being down, and a
// person told "I couldn't reach the blueprint" goes looking for an outage that
// is not there.
//
...
test("the tool reports an unconfigured blueprint with its own reason, not `unreachable`", async () => {
  ...
  assert.match(String(out.error), /not configured/i);
