/**
 * Profile Organism - Main Overview Story
 * 
 * This organism combines multiple components to create complete profile management experiences.
 * 
 * ## Structure
 * 
 * The Profile organism is organized into the following categories:
 * 
 * - **Elements**: Individual form elements and UI components (tutor profile select dropdown)
 * - **Cards**: Card components (to be added as needed)
 * - **Tables**: Table components (to be added as needed)
 * - **Modals**: Modal dialogs (to be added as needed)
 * - **Sections**: Section-level components (to be added as needed)
 * - **Pages**: Complete page-level components (tutor profile page)
 * 
...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Profile organism - higher-level components for user profile management and tutor profile selection. Navigate to each subcategory (Elements, Cards, Tables, Modals, Sections, Pages) to see individual components.',
      },
    },
  },
};
...
/**
 * Overview
 * Main overview of the Profile organism structure
 */
