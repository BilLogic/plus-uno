---
embodiment: all
summary: A structured service blueprint for the PLUS journey: how tutors, lead tutors, classroom teachers, researchers, and support staff move through the end-to-end experience
---
...
join lanes l on c.lane_id = l.id
join steps st on c.step_id = st.id
where concat_ws(' ', c.content, c.summary) ilike '%student order%';
```

**Cross-cell dependencies**
```sql
select sc.content as source, cd.kind, tc.content as target
from cell_dependencies cd
join cells sc on cd.source_cell_id = sc.id
join cells tc on cd.target_cell_id = tc.id
limit 50;
```
<!-- /ide-only -->
