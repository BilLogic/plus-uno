...
Load order is a bundle-level fact, declared once in the bundler's `SECTIONS` list. **166,642 chars from 16 files**, against an assembled budget of 170,000 (3,358 to spare), and a floor of 16,384 + 4,000 (implicit cache, GEMINI_REGION global), 146,258 above it. The floor is the minimum the cache in force will hold — Google's implicit cache on the `global` endpoint, the explicit `cachedContents` cache on a regional one — chosen by `GEMINI_REGION` in `agents/uno-bot/wrangler.toml`; a bundle cut under it ships uncached.
...
| 16 | engineering | [`docs/engineering/operations.md`](../../docs/engineering/operations.md) | 7,635 (−605 ide-only) | 166,642 | — |
