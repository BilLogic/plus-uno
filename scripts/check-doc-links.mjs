...
function markdownUnder(dirs, { exclude = [], plus = [] } = {}) {
...
for (const file of markdownUnder(LINK_DIRS, {
  exclude: [TRANSCRIPTS, BUNDLE],
  plus: ['AGENTS.md', 'CONTEXT.md', 'SETUP.md', 'README.md'],
})) {
  for (const { target } of links(file, { root: REPO_ROOT })) {
...
console.log('[check] validating backticked repo paths resolve');
...
const codeSpans = (file) => [
  ...readFileSync(path.join(REPO_ROOT, file), 'utf8').matchAll(CODE_SPAN),
].map((m) => m[1]);
...
for (const file of markdownUnder(POINTER_DIRS, {
...
console.log('[check] validating backticked bare filenames name a file that exists');
...
console.log('[check] validating AGENTS.md skills-table rows resolve to SKILL.md files');

const agentsText = readFileSync(path.join(REPO_ROOT, 'AGENTS.md'), 'utf8');
for (const ref of [...new Set([...agentsText.matchAll(/skills\/uno-[a-z-]+/g)].map((m) => m[0]))].sort()) {
  if (!existsSync(path.join(REPO_ROOT, ref, 'SKILL.md'))) {
...
console.log('[check] validating JSON index files');
...
for (const idx of REQUIRED_INDEXES) {
  const abs = path.join(REPO_ROOT, idx);
  if (!existsSync(abs)) {
...
console.log('[check] validating repo paths INSIDE the JSON indexes resolve');

const indexPaths = spawnSync('node', ['scripts/validate-index-paths.mjs'], {
  cwd: REPO_ROOT,
  encoding: 'utf8',
});
if (indexPaths.stdout) process.stdout.write(indexPaths.stdout);
if (indexPaths.stderr) process.stderr.write(indexPaths.stderr);
if (indexPaths.status !== 0) failures.push('validate-index-paths');

// ── 7. retired path shapes ──────────────────────────────────────────────────

console.log('[check] validating no old path remnants in active files');
...
console.log('[fail] validation checks failed — see details above');
process.exit(1);
