...
stateDiagram-v2
  ...
  SideEffectTool --> ProposalPosted: post proposal msg<br/>+ react :warning:<br/>+ store pending tool call in DO
  ProposalPosted --> WaitingForConfirm: subscribe to reaction_added

  WaitingForConfirm --> Confirmed: user reacts ✅<br/>(matching proposal msg ts)
  WaitingForConfirm --> Cancelled: user reacts ❌<br/>OR replies w/ correction
  WaitingForConfirm --> TimedOut: 15 min elapsed<br/>(DO TTL)

  Confirmed --> ToolFired: fire repository_dispatch<br/>react :handshake: on user msg
