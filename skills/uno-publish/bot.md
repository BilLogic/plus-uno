...
- **`shareout_post(summary, link?, reviewers?, deadline?)`** — gated (it pings people). Posts the share-out per docs/conventions/slack.md (shape, ≤3 questions + NOT-looking-for line, channel). **The bundle gate is a hard rule you must apply before invoking:** a prototype share-out missing any required bundle piece (contract: method.md) does not post; gather the missing links conversationally, and route replica creation to the IDE face (the Worker can't build Figma frames). The Worker preflight now REJECTS a prototype share-out missing the bundle (Loom + live preview + decision-log links; replica for prototypes) — if it bounces, gather the missing links conversationally; replica creation + visual diff remain IDE work. Draft the post text first; invoke once approved. `reviewers`: Slack user ids (U…) get @-mentioned — pick real people via `notion_search` (scope: "team") or the PRD Owner.
