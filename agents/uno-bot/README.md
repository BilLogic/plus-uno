...
The Worker code is only the *body*. The *brain* — guidance, persona, skills — is **baked into the Worker bundle at build time** by `npm run bundle:harness` (which reads the repo's harness sources and generates `agents/uno-bot/src/generated/harness.ts`, plus `agents/uno-bot/src/generated/references.ts` — the docs that declare `disclosure: reference`, served by name through the `read_reference` tool instead of riding every turn). Serving either costs **zero subrequests**.
...
rather than as the one escaped string `agents/uno-bot/src/generated/harness.ts` can hold. Membership is declared by
...
Because the brain is bundled, **guidance changes reach the bot on `deploy`, not on a push to `main`** (`deploy` runs `bundle:harness` first — see `agents/uno-bot/package.json`). Editing the persona/skills and pushing to `main` alone does **not** reprogram the running bot; you must `wrangler deploy`.

## Model providers — two adapters, one switch

`MODEL_PROVIDER` in `agents/uno-bot/wrangler.toml` selects which adapter answers every turn (`agents/uno-bot/src/agent/run-agent.ts` § `selectProvider` reads it — the one place it is read, asserted by `agents/uno-bot/scripts/provider-read.test.mjs`; every other caller that wants a model, the pre-send draft judge included, takes the provider that function returns):

| | `gemini` (**default / active**) | `vertex-claude` |
|---|---|---|
| Adapter | `agents/uno-bot/src/agent/providers/gemini.ts` — `GEMINI_MODEL` (`gemini-3.8-flash`) | `agents/uno-bot/src/agent/providers/claude.ts` — tiered `claude-*` on Vertex (`sonnet` default; "think hard" → `opus`; confirm/cancel → `haiku`) |
...
**Free-tier constraints are deliberate** (50 subrequests/invocation, ~10ms CPU) — see the annotated block at the top of `agents/uno-bot/wrangler.toml`; the AgentRunner processes exactly one job per alarm firing for this reason.
...
`npm run deploy` runs every gate and `bundle:harness` before wrangler sees anything, so it ships the whole `agents/uno-bot/src/` of the current branch **and** re-bakes the latest harness sources into the bundle. The chain itself is the `deploy` script in `agents/uno-bot/package.json` — read it there rather than here: it was spelled out in five places, each drifting on its own schedule, and this was the most-read and the most wrong of them (#580). Bare `npx wrangler deploy` skips all of it: no gate runs, and it ships whatever `agents/uno-bot/src/generated/harness.ts` is on disk, which is exactly the stale-brain trap this section exists to prevent. Confirm the branch's Worker code and harness are production-ready first.
