...
| **ratchet** | a check's recorded baseline, and the module that owns it (`scripts/lib/ratchet.mjs`): the READ — no check parses a record itself, and what it gates on that the module does not own comes back through `envelope()`; which direction fails; the stale-entry and placeholder-reason sweeps; and the `--update` write, a MERGE from the record on disk, so a key the module does not own survives it, the reasons three records keep beside their set. Twelve records, twelve SHAPES, one row each in `scripts/lib/ratchet-shapes.mjs`; the invariant is stated once and asserted against all twelve in `scripts/lib/ratchet-conformance.mjs`, the way **ThreadState**'s conformance suite holds two adapters equal. A recorded count may fall and a rise is a finding; an entry the run stops finding is one too, since a ratchet that only grows is a list; an absent or unreadable record fails loudly, because an empty baseline reads green | "the whitelist", "the exceptions file", "the allow-list" |
