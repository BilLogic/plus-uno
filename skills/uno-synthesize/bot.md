...
- **"draft a PRD" / "turn this into a PRD" / "file it"** → the `create_prd` flow:
  1. Draft as plain text first (structure per method § 4) and let the designer refine — do NOT call `create_prd` yet.
  2. Only on approval ("looks good, create it") → `create_prd(title, summary, sections?, acceptance_criteria?, product_pillar?, prd_type?, source_url?)` — `title` + `summary` required; gated (✅); files a card on the Design HQ → Product board in "Need PRD / Under Playground", tagged Design.
  3. `prd_type` routing: product/feature PRD → `"feature"` (default, Roadmap board); DS *component* PRD → `"ds-component"` (DS Component PRDs DB). If genuinely unclear, ask.
  4. Undo via `delete_prd(notion_url)` — gated; pass the Notion link posted at creation (recoverable from Notion trash).
...
- After the PRD is filed → **uno-prototype** (`implement_design`) to scaffold from a Figma frame; a natural sequence is create_prd → implement_design.
- People-sourcing earlier in the flow → **uno-research** (`find_experts`); deep multi-file codebase research → the in-IDE `skills/uno-research`.
