...
step "Paste agents/uno-bot/slack-app-manifest.yaml — the WHOLE file — and save."
warn "That is the only paste. Do NOT also paste slack-app-manifest-commands.yaml:"
note "  it is a generated FRAGMENT that belongs under `features:`, not a manifest,"
note "  and pasting it whole is what produces Slack's"
note "      Invalid additional property: slash_commands"
note "  Its own header says \"Paste under features:\". All six of its commands are"
note "  already inside the main manifest, which carries nine — the fragment is a"
note "  strict subset, so pasting it adds nothing even when merged correctly."
