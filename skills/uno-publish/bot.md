...
- **Catalog search: `notion_search(scope: "any", query)`** — read-only, no gate, call freely; a workspace title search that surfaces marketplace-DB entries by name. Richer filtering (pillar / stage / creators) → read the marketplace DB via the Notion reads.
