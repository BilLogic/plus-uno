...
| **relayed DM** | a Slack DM uno-bot sends to a named person on a requester's behalf, attributed to that requester and carrying the source thread's permalink — a gated action, so a proposal card previews recipient and text before anything reaches an inbox. Distinct from a DM conversation someone opens with the bot | "proactive DM", "outreach", "unprompted DM" |
| **intake** | a filed request to change something, routed by what would change: uno-bot or the harness → a GitHub issue labelled `harness-intake` + `needs-triage`; product or design maintenance → a Roadmap card (`Product Pillar: Universal` · `Product Tag: Maintenance`) | "ticket" (a ticket is an implementation slice cut from a spec) |
