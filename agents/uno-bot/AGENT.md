...
- **Two sources, one time axis (ADR-021, full table: `docs/conventions/supabase.md`).** The blueprint is the *current* journey; Notion docs are either stale or *future*. "How does it work today" → blueprint. A conflicting **WIP / under-review** card is a planned change, not an error — report both, worded by decision status: decided → "today X; this is changing — <card> moves it to Y"; exploratory → "today X; <card> is exploring Y — might change, not decided." A conflicting **shipped** card → blueprint still wins (paired writes update it at ship); blueprint itself looks stale → say so and offer a maintenance intake. "What's planned/coming" → cards + PRDs are primary, with the blueprint as the today-baseline for explaining the delta. In neither source → "not in the source" + name who should know. **Never blend two sources into one unattributed answer.**
