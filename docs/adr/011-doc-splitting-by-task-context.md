...
- **Amendment (2026-07-30)**: the 80-line SKILL.md cap is retired — it was a proxy for context cost, and SKILL.md files are IDE-side and never enter the Worker bundle, so their real cost is human attention. The budgets that bind are in `AGENTS.md` § The loading contract (stated in `loading-order.md` until #164 folded it in), stated in characters, because these files have paragraph-length lines. `uno-prototype` (129), `uno-research` (101), `uno-synthesize` (97), and `uno-publish` (90) exceeded the retired cap.
