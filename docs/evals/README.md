...
Populated 2026-07-07: 7 rubrics (six skills + bot-answer) rebuilt from the Notion 📊 Evals doc's metric targets, and 7 scenario sets (six skills, evals-first, + the bot regression prompts R1–R12, migrated from the former `agents/uno-bot/REGRESSION.md`, since removed). NOTE: `fixtures/uno-bot-cases.json` — what the `uno-bot — evals` Action actually runs — carries **26 cases**, all executed by `agents/uno-bot/scripts/run-evals.mjs`: R1–R12 plus R20 (13 R-cases), P1–P6 (prototype-method regression guards; P2/P5 rotate the provenance coverage added 2026-08-31; P6, added 2026-09-04 with #419, proves a pasted Figma URL reaches `prototype_scaffold` from the tool descriptions alone) and S1–S3, T1–T2 (tier and thinking level, 2026-09-04), M1 (a maintain turn reaching its disclosed method through `read_reference`, 2026-09-05 with #423), and D1 (a publish turn reaching `uno-publish/method` the same way, 2026-09-05 with #424; the other methods disclosed with it have no case of their own yet). Its LLM-judge answers live only in the encrypted sibling fixture and are loaded with an Actions secret, so the bot under test cannot retrieve its own grading instructions through `github_read`. This line read "16 total" until 2026-08-27; R20 and S1–S3 were added without it, which is how a count in prose drifts from a count in a file (#249). bot-answer dimensions D1/D4 were never recorded anywhere recoverable; their definitions are reconstructed as inferred-and-canonical in the rubric (2026-07-08).
