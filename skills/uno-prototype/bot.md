...
Propose gated implementation runs: DS-library component updates (`component_implement`) and new prototypes prototypes (`prototype_scaffold`). Both are side-effect tools — every invocation goes through the confirmation gate; the Worker stages the proposal and holds for ✅.
...
- **`prototype_scaffold(figma_url, notion_prd_url?, slug?, notes?)`** — fires `figma-implement-design.yml`; scaffolds a new `prototypes/{slug}/` and opens a real draft PR. Use for "implement this design <figma.com/…>", "build a prototype from this Figma frame", "scaffold a prototypes for this screen". NOT for DS-library component updates.
