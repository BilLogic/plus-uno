...
## ADR-022: Count subrequests, don't estimate them (2026-07-30)

**Decision.** The Worker's subrequest budget is enforced against a **measured** counter, not an estimate table. `src/net.ts` owns the single outbound `countedFetch` and keeps a per-invocation count in AsyncLocalStorage; Durable Object stub calls (invisible to `fetch`) are charged explicitly. `scripts/check-fetch.mjs` fails the build on a bare `fetch(` anywhere in `src/` outside `net.ts`, so a new call site cannot be silently uncounted. One estimate necessarily remains — a tool's cost is unknowable until it runs — so the gate is **measured-past + bounded-future**: real spend so far, plus `maxToolSubrequests(name)`, a worst-case bound derived from each tool's page caps and fan-out. Every tool call's actual spend is compared to its bound and logged (`[budget]`), and exceeding a bound logs a warning naming the constant to raise. `PRE_GROUNDING_OVERHEAD` and `GROUNDING_BUDGET` are retired in favour of `LOOKUP_CEILING = SUBREQUEST_CAP - DELIVERY_RESERVE` (38). That is the same total the old pair added up to (10 + 28) — the gain is accuracy, not headroom: the old split could overshoot 50 whenever real startup exceeded the assumed 10, and the new one can't. The gate also covers loop iterations, not just lookups: refusing a lookup left the model free to keep requesting tools, and while each refusal is free the model round-trip carrying it is not, so `MAX_ITERATIONS` of them walked through the delivery reserve and killed the post.

**Why.** The prior design deliberately chose estimates over counting ("NOT real fetch-counting: this touches only the loop files, so there's no 'missed a fetch path' failure mode"). That trade turned out to buy nothing: the estimates drifted instead. `notion_search` was priced at 4 while its `apps` scope really spent 6 (2 directory pages + up to 4 power-user title lookups), and nothing could detect it, because nothing measured. The failure mode is asymmetric and severe — an under-estimate lets the gate wave a call through that carries the invocation past Cloudflare's hard cap of 50, and since posting the reply is itself a subrequest, the turn dies with no message at all (👀-then-silence, live 2026-07-10 and 2026-07-13). The "missed a fetch path" risk the original decision feared is real but is a *build-time* problem, which a guard script solves; estimate drift is a *runtime* problem that no amount of care prevents. Also: raising `CATALOG_MAX_PAGES` 2→5 would have widened the existing gap on the exact tool AGENT.md's named-artifact rule routes the bot toward.

**Consequences.** ~35 call sites now import `countedFetch`; `http.ts`'s `fetchWithTimeout` is an alias for it, so the timeout guard and the counter can't be taken separately. `@types/node` is a devDependency and `tsconfig` includes the `node` types (for `node:async_hooks`; `nodejs_compat` was already on). `/debug/eval` returns `subrequests` and a per-host breakdown, so an eval run sees how close a turn came to the cap instead of learning by outage. Bounds are now a maintained constant with a feedback loop rather than a guess with none — a bound that reality exceeds shows up as a logged warning, and a bound chronically far above reality shows up as wasted research headroom in the same line.
