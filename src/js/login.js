...
function initializeLoginPage() {
    ...
    // Add empty block for Clever icon placeholder (after button is created)
    // Replace with actual Clever logo image when source is available
    if (cleverButton && cleverButton.classList.contains("pbtn")) {
        ...
        if (cleverButtonContent) {
            const iconPlaceholder = document.createElement("span");
            iconPlaceholder.className = "clever-icon-placeholder";
            const textSpan = cleverButtonContent.querySelector("span");
            if (textSpan) {
                cleverButtonContent.insertBefore(iconPlaceholder, textSpan);
            }
        }
    }
