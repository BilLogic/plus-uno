...
| Marketplace add/edit | `repository_dispatch` | uno-publish (registration) | — | `marketplace-add.yml` / `marketplace-edit.yml` | GHA (Slack notice only) | Bill | ❌ stub — logs payload + posts a construction notice; nothing dispatches it (no Worker tool); real implementation planned |
