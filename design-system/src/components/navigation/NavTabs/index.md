<!-- DO NOT EDIT BY HAND. Generated by npm run generate:component-docs -->
<!-- Every fact below is derived from source. To change one, change the source. -->

# NavTabs

**Import:** `import { NavTabs } from '@/components';`

**Source:** `design-system/src/components/navigation/NavTabs/NavTabs.jsx`

**Stories:** `design-system/src/components/navigation/NavTabs/NavTabs.stories.jsx` — Content, Layout, InteractionStates, Overview, Semantics, Interactive

**Storybook page:** `design-system/src/components/navigation/NavTabs/NavTabs.mdx`

## Props

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `children` | node | — | — |
| `alignment` | enum | `'left'` | Horizontal alignment of nav items |
| `className` | string | `''` | — |
| `defaultActiveKey` | any | — | — |
| `activeKey` | any | — | — |
| `onSelect` | func | — | — |

## Variants

**`alignment`** — `left` · `center` · `right` · `justified`

Anything not listed is not a valid value.

## Tokens touched

`--color-focus-ring` · `--color-on-surface` · `--color-on-surface-state-08` · `--color-on-surface-state-12` · `--color-on-surface-state-16` · `--color-outline-variant` · `--color-primary` · `--color-primary-border` · `--color-primary-state-08` · `--color-primary-state-12` · `--color-primary-state-16` · `--color-secondary-border` · `--color-secondary-state-08` · `--color-secondary-text` · `--font-family-body` · `--font-line-height-body1` · `--font-size-body1` · `--font-weight-body1-semibold` · `--font-weight-normal` · `--size-element-gap-md` · `--size-element-pad-x-lg` · `--size-element-pad-y-md` · `--size-element-radius-md` · `--size-element-stroke-md` · `--size-element-stroke-sm` · `--size-section-gap-sm`

From `design-system/src/components/navigation/NavTabs/NavTabs.scss`. Override these through the token layer, never with a literal.

## Sub-components

### `NavTabs.Item`

Implemented as `NavTabsItem`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `children` | node | — | — |
| `active` | bool | — | — |
| `disabled` | bool | — | — |
| `href` | string | — | — |
| `onClick` | func | — | — |
| `className` | string | — | — |
| `as` | elementType | — | — |
| `eventKey` | string or number | — | — |

### `NavTabs.Panel`

Implemented as `NavTabsPanel`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `children` | node | — | — |
| `eventKey` | string or number | — | — |

### `NavTabs.Dropdown`

Implemented as `NavTabsDropdown`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `title` | node (required) | — | — |
| `children` | node | — | — |
| `active` | bool | — | — |
| `disabled` | bool | — | — |
| `className` | string | — | — |
| `id` | string | — | — |
| `menuVariant` | string | — | — |

## Related

Same group: `Breadcrumb` · `NavPills` · `Pagination` · `Scrollspy` · `SidebarTab`

<!-- authored coverage: whenToUse=yes correctIncorrect=no accessibility=yes -->

> Authored guidance is partial — missing: correct/incorrect (#166).
