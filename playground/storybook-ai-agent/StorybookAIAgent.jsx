...
const LogoContainer = ({ size = 'default', className = '', variant = 'default' }) => {
    ...
    const IconComponent = variant === 'fab-open' ? Icons.FabOpen : variant === 'fab-closed' ? Icons.FabClosed : Icons.Sparkles;
    return (
...
const QUICK_ACTIONS = [
    ...
    { id: 'skills', label: 'Agent Skills', aliases: ['agent skills'], Icon: Icons.Skills, color: 'var(--color-advocacy-text)', bg: 'var(--color-advocacy-container)' },
];
...
function getComponentEntryFromStoryId(storyId) {
    if (!storyId) return null;
    const slug = storyId.split('--')[0] || storyId;
    const pathForm = slug.includes('/') ? slug : slug.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('/');
    return STORYBOOK_COMPONENTS.find(e =>
        e.storyId === pathForm || e.storyId === storyId || e.storyId.toLowerCase() === pathForm.toLowerCase()
    ) || null;
}
...
const NAV_INDEX = [
    ...
    { storyId: 'Components/Alert', storyName: 'Overview', category: 'Component', label: 'Alert', terms: ['alert', 'alerts'] },
    { storyId: 'Components/Accordion', storyName: 'Overview', category: 'Component', label: 'Accordion', terms: ['accordion'] },
    { storyId: 'Components/Breadcrumb', storyName: 'Overview', category: 'Component', label: 'Breadcrumb', terms: ['breadcrumb'] },
    { storyId: 'Components/Dropdown', storyName: 'Overview', category: 'Component', label: 'Dropdown', terms: ['dropdown'] },
    { storyId: 'Components/NavTabs', storyName: 'Overview', category: 'Component', label: 'NavTabs', terms: ['nav', 'tabs', 'navtabs'] },
    { storyId: 'Components/Pagination', storyName: 'Overview', category: 'Component', label: 'Pagination', terms: ['pagination'] },
    { storyId: 'Components/Progress', storyName: 'Overview', category: 'Component', label: 'Progress', terms: ['progress'] },
    { storyId: 'Components/Tooltip', storyName: 'Overview', category: 'Component', label: 'Tooltip', terms: ['tooltip'] },
    { storyId: 'Components/Popover', storyName: 'Overview', category: 'Component', label: 'Popover', terms: ['popover'] },
    { storyId: 'Components/ListGroup', storyName: 'Overview', category: 'Component', label: 'ListGroup', terms: ['list', 'listgroup'] },
    { storyId: 'Components/Spinner', storyName: 'Overview', category: 'Component', label: 'Spinner', terms: ['spinner', 'loading'] },
    { storyId: 'Specs/Admin/Tutor Admin/Pages/TutorTrainingProgressPage', storyName: 'Overview', category: 'Page', label: 'Tutor Training Progress', terms: ['tutor training progress', 'tutor training progress page', 'training progress', 'tutor training', 'tutor progress'] },
    { storyId: 'Specs/Admin/Tutor Admin/Pages/TutorPerformancePage', storyName: 'Overview', category: 'Page', label: 'Tutor Performance', terms: ['tutor performance', 'performance'] },
    { storyId: 'Playground/Ashley/Storybook AI Agent', storyName: 'Demo', category: 'Playground', label: 'Storybook AI Agent', terms: ['ai agent', 'agent'] },
    { storyId: 'Styles/Colors', storyName: 'Overview', category: 'Tokens', label: 'Colors', terms: ['colors', 'color tokens'] },
    { storyId: 'Styles/Typography', storyName: 'TypographyScales', category: 'Tokens', label: 'Typography', terms: ['typography', 'type', 'font'] },
    { storyId: 'Styles/Elevation', storyName: 'Overview', category: 'Tokens', label: 'Elevation', terms: ['elevation', 'shadow'] },
    { storyId: 'Styles/Layout', storyName: 'Overview', category: 'Tokens', label: 'Layout', terms: ['layout', 'spacing', 'spacing tokens'] },
];

/* Components that exist in Storybook (for Component Guide — no hallucination) */
const STORYBOOK_COMPONENTS = NAV_INDEX.filter(e => e.category === 'Component');

function matchComponentFromToken(token) {
    if (!token || token.length < 2) return null;
    const q = normalizeForMatch(token);
    const scored = STORYBOOK_COMPONENTS.map(entry => ({
        entry,
        score: scoreMatch(q, entry)
    })).filter(x => x.score > 0);
...
function parseComponentGuideQuery(text) {
    ...
    if (vsMatch) {
        const [left, right] = t.split(vsMatch[0]).map(s => s.trim()).filter(Boolean);
    ...
    if (whenMatch) {
        const rest = whenMatch[1].trim().replace(/\?/g, '');
        if (rest.includes(' vs ') || rest.includes(' or ')) {
            const parts = rest.split(/\s+vs\.?\s+|\s+or\s+/).map(s => s.trim()).filter(Boolean);
...
function hasNavigationIntent(text) {
    const t = text.toLowerCase().trim();
    return /\b(where|open|go to|show|find|take me to)\b/i.test(t) || /^(buttons?|cards?|modal|badge|training progress|spacing|tokens?)$/i.test(t);
}
...
function scoreMatch(query, entry) {
    ...
    const termsStr = entry.terms.join(' ');
    for (const term of entry.terms) {
        const t = normalizeForMatch(term);
        if (t === q || q === t) return 100;
        if (q.includes(t) || t.includes(q)) best = Math.max(best, 80);
        if (labelNorm.includes(q) || q.includes(labelNorm)) best = Math.max(best, 90);
    }
    for (const w of qWords) {
        const syn = NAV_SYNONYMS[w];
        const expanded = syn ? [w, syn] : [w];
        for (const x of expanded) {
            if (labelNorm.includes(x) || termsStr.includes(x)) best = Math.max(best, 70);
        }
        for (const term of entry.terms) {
            if (term.includes(w) || (NAV_SYNONYMS[w] && term.includes(NAV_SYNONYMS[w]))) best = Math.max(best, 60);
        }
    }
    if (labelNorm.includes(q) || termsStr.includes(q)) best = Math.max(best, 85);
    // Prefer Page when query contains "page" so "tutor training progress page" goes to the page not Progress component
    if (qWords.includes('page') && entry.category === 'Page' && best > 0) best += 10;
    // Prefer longer label match when query is multi-word (e.g. "tutor training progress" over "progress")
    if (qWords.length >= 2 && labelNorm.includes(q.replace(/\bpage\b/, '').trim()) && best > 0) best += 5;
    return best;
}

function searchNavigation(query, limit = 5) {
    const q = extractSearchQuery(query);
    if (!q) return [];
    const withScores = NAV_INDEX.map(entry => ({ entry, score: scoreMatch(q, entry) })).filter(x => x.score > 0);
    withScores.sort((a, b) => b.score - a.score);
    return withScores.slice(0, limit).map(x => x.entry);
}
...
const buildResponse = (actionId, pageContext) => {
    switch (actionId) {
        ...
        case 'skills':
            return (
                <div>
                    <p style={{ fontWeight: 600, marginBottom: 8 }}>🎯 The 5 Main Skills</p>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                        <li style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            🏗️ <strong>Building</strong> — Build NEW components from Figma
                        </li>
                        <li style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            🔧 <strong>Maintaining</strong> — Update/fix EXISTING components
                        </li>
                        <li style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            🎨 <strong>Prototyping</strong> — Create quick visual prototypes
                        </li>
                        <li style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            💡 <strong>Design Consulting</strong> — Get design guidance
                        </li>
                        <li style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            📚 <strong>Learn PLUS</strong> — Learn about PLUS design system
                        </li>
                    </ul>
                </div>
            );
...
const StorybookAIAgent = ({ pageContext = 'Tutor Training Progress Page', userName = 'Ashley' }) => {
    ...
    /* Smart Navigation: intent → search → single nav / multi picker / no match. No intro when auto-detected. */
    const runSmartNavigation = (text) => {
        const matches = searchNavigation(text, 5);
        if (matches.length === 0) {
            respond(<p>Nothing found. Try a different keyword.</p>);
            return;
        }
        if (matches.length === 1) {
            const entry = matches[0];
            navigateToEntry(entry);
            respond(<p>Found <strong>{entry.label}</strong> under {entry.category} → {entry.label}. Opening it now…</p>);
            return;
        }
        setNavMatches(matches);
        setSelectedNavIndex(0);
        respond({ type: 'nav-picker', text: 'I found multiple matches:' });
    };
    ...
    const handleSend = () => {
        ...
        /* Component Guide: parse component name(s), match against Storybook, structured or comparison response */
        if (hasComponentGuideIntent(text)) {
            const parsed = parseComponentGuideQuery(text);
            if (parsed) {
                if (parsed.type === 'context') {
                    const storyId = getCurrentStoryId();
                    const entry = getComponentEntryFromStoryId(storyId);
                    if (entry) {
                        respond(renderSingleComponentGuide(entry));
                    } else {
                        respond(<p>No matching component found in Storybook.</p>);
                    }
                    return;
                }
                if (parsed.type === 'comparison' && parsed.tokens.length >= 2) {
                    const entryA = matchComponentFromToken(parsed.tokens[0]);
                    const entryB = matchComponentFromToken(parsed.tokens[1]);
                    if (entryA && entryB) {
                        const usageEntry = matchUsageGuide(text);
                        if (usageEntry) {
                            respond(usageEntry.answer);
                        } else {
                            respond(renderComparisonGuide(entryA, entryB));
                        }
                    } else {
                        respond(<p>No matching component found in Storybook.</p>);
                    }
                ...
                if (parsed.type === 'single' && parsed.tokens.length >= 1) {
                    const entry = matchComponentFromToken(parsed.tokens[0]);
                    if (entry) {
                        respond(renderSingleComponentGuide(entry));
                    } else {
                        respond(<p>No matching component found in Storybook.</p>);
        ...
        if (hasUsageIntent(text)) {
            const entry = matchUsageGuide(text);
            if (entry) {
                respond(entry.answer);
            } else {
                respond(<p>No matching component found in Storybook.</p>);
            }
            return;
        ...
        /* Typed quick-action keyword: show same reply as clicking that button */
        const quickActionId = matchQuickActionByKeyword(text);
        if (quickActionId) {
            if (quickActionId === 'find') {
                setMessages(p => [...p, { role: 'bot', content: <SmartNavIntroMessage /> }]);
                setTimeout(() => inputRef.current?.focus(), 0);
            } else if (quickActionId === 'usage') {
                setMessages(p => [...p, { role: 'bot', content: <UsageGuideIntroMessage /> }]);
                setTimeout(() => inputRef.current?.focus(), 0);
            } else {
                respond(buildResponse(quickActionId, pageContext));
            }
            return;
        }
    ...
    if (isHidden) {
        return <div className="sb-ai-agent" aria-hidden="true" style={{ position: 'fixed', inset: 0, pointerEvents: 'none', visibility: 'hidden', overflow: 'hidden', width: 0, height: 0 }} />;
    }
